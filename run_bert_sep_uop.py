...
from nup_lm_finetuning.modeling_bert import BertForSequenceClassificationUOP
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, order=None, label=None):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, shuffle_idx, eou_ids, label_id):
...
def convert_examples_to_features(examples, label_list, max_context_length, max_response_length, tokenizer, max_utterance):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # if len(total_chunks) != len(tokens_b):
        #     print("aaa")
        # assert len(total_chunks) == len(tokens_b)
...
def main():
    ...
    model = BertForSequenceClassificationUOP.from_pretrained(args.bert_model, num_labels=num_labels)
    ...
    if args.do_train:
        ...
        try:
            ...
            train_features = convert_examples_to_features(
                train_examples, label_list, args.max_context_length, args.max_response_length, tokenizer, args.max_utterance)
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_order_ids, all_eou_ids, all_label_ids)
        if args.local_rank == -1:
        ...
        try:
            ...
            eval_features = convert_examples_to_features(
                eval_examples, label_list, args.max_context_length, args.max_response_length, tokenizer, args.max_utterance)
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:
        ...
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_order_ids, all_eou_ids, all_label_ids)
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_mask, segment_ids, order_ids, eou_ids, label_ids = batch

                # define a new function to compute loss values for both output_modes
                loss = model(input_ids=input_ids, attention_mask=input_mask, token_type_ids=segment_ids, order_ids=order_ids, eou_ids=eou_ids, labels=label_ids)
                # logits = output[0]
            ...
            eval_model = BertForSequenceClassificationUOP.from_pretrained(args.output_dir, state_dict=model_state_dict)
            ...
            for input_ids, input_mask, segment_ids, order_ids, eou_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                with torch.no_grad():
                    logits = eval_model(input_ids=input_ids, attention_mask=input_mask, token_type_ids=segment_ids, order_ids=order_ids, eou_ids=eou_ids, labels=None)
                    # logits = output[0]
        ...
        model_state_dict = torch.load(output_model_file)
        model = BertForSequenceClassificationUOP.from_pretrained(args.output_dir, state_dict=model_state_dict)
        # need check the eval model
        # model.resize_token_embeddings(len(tokenizer))
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_mask, segment_ids, order_ids, eou_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                logits = model(input_ids, segment_ids, input_mask, order_ids, eou_ids, labels=None)
                # logits = output[0]
            # create eval loss and other metric required by the task
