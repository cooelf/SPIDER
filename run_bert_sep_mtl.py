...
from nup_lm_finetuning.modeling_bert import BertForSequenceClassificationMTL
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, order=None, svo_ids=None, label=None):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, shuffle_idx, eou_ids, svo_ids, label_id):
        self.input_ids = input_ids
        self.input_mask = input_mask
        self.segment_ids = segment_ids
        self.shuffle_idx = shuffle_idx
        self.eou_ids = eou_ids
        self.svo_ids = svo_ids
    ...
    def _read_data(cls, input_file):
        ...
        with open(input_file, "r", encoding="utf-8") as f:
            ...
            for line in f:
                ...
                lable = parts[0]
                context_part = parts[1:-1]
                # shuffle_idx = np.random.permutation(np.arange(len(context_part)))
                # shuffle_idx = shuffle_idx.tolist()
                # context_part = [context_part[i] for i in shuffle_idx]
                message = ""
    ...
    def _read_douban_data(cls, input_file):
        ...
        with open(input_file, "r", encoding="utf-8") as f:
            ...
            for ids,line in enumerate(f):
                ...
                shuffle_idx_list.append(shuffle_idx)
                if ids % 10 == 9:
                    if label_any_1 == 1:
                        for lable,message,response, shuffle_idx in zip(label_list, message_list, response_list, shuffle_idx_list):
                            data = {"y": lable, "m": message, "r": response}
                            lines.append(data)
                    label_any_1 = 0
                    label_list = []
...
def convert_examples_to_features(examples, label_list, max_context_length, max_response_length, tokenizer, max_utterance, max_svo_len):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens_a = tokenizer.tokenize(example.text_a) #response
        tokens_b = tokenizer.tokenize(example.text_b) #context
        # Modifies `tokens_a` and `tokens_b` in place so that the total
        # length is less than the specified length.
        # Account for [CLS], [SEP], [SEP] with "- 3"
        if len(tokens_a) < 1:
            print("debug")
        _truncate_seq_pair(tokens_b, tokens_a, max_context_length-2, max_response_length-1)
        max_seq_length = max_context_length + max_response_length
        ...
        # utterance order ids
        total_chunks = " ".join(tokens_b)
        total_chunks = total_chunks.split("[EOU]")
        total_chunks = [item for item in total_chunks if item.strip()]
        shuffle_idx = np.random.permutation(np.arange(len(total_chunks)))
        shuffle_idx = shuffle_idx.tolist()
        total_chunks = [total_chunks[i] + " [EOU]" for i in shuffle_idx]
        total_chunks = " ".join(total_chunks)
        total_chunks = total_chunks.split()

        utterance = []
        offset = len(tokens) - 1
        eou_ids = []
        for token_item in total_chunks:
            if token_item != "[EOU]":
        ...
        tokens_b = total_chunks

        if tokens_b:
            tokens += tokens_b + ["[SEP]"]
            segment_ids += [1] * (len(tokens_b) + 1)

        while len(shuffle_idx) > max_utterance:
            del eou_ids[0]
            start_idx = shuffle_idx[0]
            new_shuffle_idx = []
            for idx, left_idx in enumerate(shuffle_idx[1:]):
                if left_idx > start_idx:
                    left_idx = left_idx - 1
                new_shuffle_idx.append(left_idx)
            shuffle_idx = new_shuffle_idx

        assert len(shuffle_idx) == len(eou_ids), print(len(shuffle_idx), len(eou_ids))
        ...
        svo_ids += [[-100, -100, -100]] * (max_svo_len - len(svo_ids))
        ...
        if ex_index < 5:
            ...
            logger.info(
                "shuffle_idx: %s" % " ".join([str(x) for x in shuffle_idx]))
            logger.info(
                "eou_ids: %s" % " ".join([str(x) for x in eou_ids]))
            logger.info(
                "svo_ids: %s" % " ".join([str(x) for x in svo_ids]))
...
def _truncate_seq_pair(tokens_a, tokens_b, max_context_len, max_response_len):
...
def main():
    ...
    parser.add_argument("--uop_weight",
                        default=1.0,
                        type=float,
    ...
    parser.add_argument("--max_response_length",
                        default=128,
                        type=int,
                        help="The maximum total input sequence length after WordPiece tokenization. \n"
                             "Sequences longer than this will be truncated, and sequences shorter \n"
    ...
    model = BertForSequenceClassificationMTL.from_pretrained(args.bert_model)
    ...
    if args.do_train:
        cached_train_features_file = args.data_dir + '_{0}_{1}_{2}_{3}_{4}_{5}'.format(
            ...
            str(args.max_context_length), str(args.max_response_length), str(args.cache_flag))
        train_features = None
        try:
            ...
            train_features = convert_examples_to_features(
                train_examples, label_list, args.max_context_length, args.max_response_length, tokenizer, args.max_utterance, args.max_svo_len)
        ...
        all_order_ids = torch.tensor([f.shuffle_idx for f in train_features], dtype=torch.long)
        all_eou_ids = torch.tensor([f.eou_ids for f in train_features], dtype=torch.long)
        all_svo_ids = torch.tensor([f.svo_ids for f in train_features], dtype=torch.long)
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_order_ids, all_eou_ids, all_svo_ids, all_label_ids)
        if args.local_rank == -1:
        ...
        try:
            ...
            eval_features = convert_examples_to_features(
                eval_examples, label_list, args.max_context_length, args.max_response_length, tokenizer, args.max_utterance, args.max_svo_len)
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:
        ...
        all_order_ids = torch.tensor([f.shuffle_idx for f in eval_features], dtype=torch.long)
        all_eou_ids = torch.tensor([f.eou_ids for f in eval_features], dtype=torch.long)
        all_svo_ids = torch.tensor([f.svo_ids for f in eval_features], dtype=torch.long)
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_order_ids, all_eou_ids, all_svo_ids, all_label_ids)
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_mask, segment_ids, order_ids, eou_ids, svo_ids, label_ids = batch

                # define a new function to compute loss values for both output_modes
                loss = model(input_ids=input_ids, attention_mask=input_mask, token_type_ids=segment_ids, order_ids=order_ids, eou_ids=eou_ids, svo_ids=svo_ids, labels=label_ids)
                # logits = output[0]
            ...
            eval_model = BertForSequenceClassificationMTL.from_pretrained(args.output_dir, state_dict=model_state_dict)

            #need check the eval model
            ...
            for input_ids, input_mask, segment_ids, order_ids, eou_ids, svo_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
                input_ids = input_ids.to(device)
                input_mask = input_mask.to(device)
                segment_ids = segment_ids.to(device)
                order_ids = order_ids.to(device)
                eou_ids = eou_ids.to(device)
                svo_ids = svo_ids.to(device)
                label_ids = label_ids.to(device)

                with torch.no_grad():
                    logits = eval_model(input_ids=input_ids, attention_mask=input_mask, token_type_ids=segment_ids, order_ids=order_ids, eou_ids=eou_ids, svo_ids=svo_ids, labels=None)
                    # logits = output[0]
        ...
        model_state_dict = torch.load(output_model_file)
        model = BertForSequenceClassificationMTL.from_pretrained(args.output_dir, state_dict=model_state_dict)
        # need check the eval model
        # model.resize_token_embeddings(len(tokenizer))
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_mask, segment_ids, order_ids, eou_ids, svo_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                logits = model(input_ids, segment_ids, input_mask, order_ids, eou_ids, svo_ids, labels=None)
                # logits = output[0]
            # create eval loss and other metric required by the task
