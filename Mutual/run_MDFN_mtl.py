...
from modeling import (electra_mtl, roberta_mtl, bert_mtl)
...
MODEL_CLASSES = {
    'bert': (BertConfig, bert_mtl, BertTokenizer),
    'roberta': (RobertaConfig, roberta_mtl, RobertaTokenizer),
    'electra': (ElectraConfig, electra_mtl, ElectraTokenizer)
}
...
class InputFeatures(object):
    ...
    def __init__(self, example_id, choices_features, label):
        ...
        self.choices_features = [
            ...
            for input_ids, input_mask, segment_ids, sep_pos, turn_ids, shuffle_idx, svo_ids in choices_features
        ]
        self.label = label
...
def convert_examples_to_features(examples, label_list, max_seq_length, max_utterance_num, max_svo_len,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for ending_idx, (text_a, text_b) in enumerate(zip(example.text_a, example.text_b)):
            ...
            for idx, text in enumerate(text_b):
                if len(text.strip()) > 0:
                    tokens_b.extend(tokenizer.tokenize(text) + ["[EOU]"])
            ...
            total_chunks = tokens_b_raw.split("[EOU]")[-max_utterance_num - 1:-1]
            ...
            sep_pos += [-100] * (max_utterance_num - len(sep_pos))
            shuffle_idx += [-100] * (max_utterance_num - len(shuffle_idx))
            assert len(sep_pos) == max_utterance_num
            assert len(input_ids) == max_seq_length
            assert len(input_mask) == max_seq_length
            assert len(segment_ids) == max_seq_length

            choices_features.append((input_ids, input_mask, segment_ids, sep_pos, turn_ids, shuffle_idx, svo_ids))
        ...
        if ex_index < 10:
            ...
            for choice_idx, (input_ids, input_mask, segment_ids, sep_pos, turn_ids, shuffle_idx, svo_ids) in enumerate(choices_features):
                ...
                logger.info("shuffle_idx: %s" % " ".join([str(x) for x in shuffle_idx]))
                logger.info("svo_ids: %s" % " ".join([str(x) for x in svo_ids]))
...
def main():
    ...
    tr_loss = 0
    if args.do_train:
        # cached_train_features_file = args.data_dir + '_{0}_{1}_{2}_{3}_{4}_{5}_uop'.format(
        ...
        train_features = convert_examples_to_features(
            train_examples, label_list, args.max_seq_length, args.max_utterance_num, args.max_svo_len, tokenizer, output_mode)
        ...
        all_shuffle_idx = torch.tensor(select_field(train_features, 'shuffle_idx'), dtype = torch.long)
        all_svo_ids = torch.tensor(select_field(train_features, 'svo_ids'), dtype = torch.long)
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sep_pos, all_turn_ids, all_shuffle_idx, all_svo_ids, all_label_ids)
        ...
        eval_features = convert_examples_to_features(
            eval_examples, label_list, args.max_seq_length, args.max_utterance_num, args.max_svo_len, tokenizer, output_mode)
            # if args.local_rank == -1 or torch.distributed.get_rank() == 0:
        ...
        all_shuffle_idx = torch.tensor(select_field(eval_features, 'shuffle_idx'), dtype = torch.long)
        all_svo_ids = torch.tensor(select_field(eval_features, 'svo_ids'), dtype=torch.long)
        ...
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sep_pos, all_turn_ids, all_shuffle_idx, all_svo_ids, all_label_ids)
