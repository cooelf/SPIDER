...
from modeling import (bert_svo, roberta_svo, electra_svo)
...
MODEL_CLASSES = {
    'bert': (BertConfig, bert_svo, BertTokenizer),
    'roberta': (RobertaConfig, roberta_svo, RobertaTokenizer),
    'electra': (ElectraConfig, electra_svo, ElectraTokenizer)
}
...
class InputFeatures(object):
    ...
    def __init__(self, example_id, choices_features, label):
        ...
        self.choices_features = [
            ...
            for input_ids, input_mask, segment_ids, sep_pos, turn_ids, svo_ids in choices_features
        ]
        self.label = label
...
def convert_examples_to_features(examples, label_list, max_seq_length, max_utterance_num, max_svo_len,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for ending_idx, (text_a, text_b) in enumerate(zip(example.text_a, example.text_b)):
            ...
            tokens_b = []
            current_pos = 0
            for toks in tokens_b_raw.split("[EOU]")[-max_utterance_num - 1:-1]:
            ...
            sep_pos += [0] * (max_utterance_num - len(sep_pos))
            ...
            choices_features.append((input_ids, input_mask, segment_ids, sep_pos, turn_ids, svo_ids))
        ...
        if ex_index < 5:
            ...
            for choice_idx, (input_ids, input_mask, segment_ids, sep_pos, turn_ids, svo_ids) in enumerate(choices_features):
...
def main():
    ...
    if args.do_train:
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sep_pos, all_turn_ids, all_svo_ids, all_label_ids)
        ...
        # cached_train_features_file = args.data_dir + '_{0}_{1}_{2}_{3}_{4}_{5}_svo'.format(
        ...
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sep_pos, all_turn_ids, all_svo_ids, all_label_ids)
