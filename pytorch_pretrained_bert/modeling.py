...
class BertForPreTrainingSVO(BertPreTrainedModel):
    def __init__(self, config):
        super(BertForPreTrainingSVO, self).__init__(config)
        self.bert = BertModel(config)
        self.cls = BertPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, next_sentence_label=None, svo_ids=None):
        sequence_output, pooled_output = self.bert(input_ids, token_type_ids, attention_mask,
                                                   output_all_encoded_layers=False)
        prediction_scores, seq_relationship_score = self.cls(sequence_output, pooled_output)
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), masked_lm_labels.view(-1))
            next_sentence_loss = loss_fct(seq_relationship_score.view(-1, 2), next_sentence_label.view(-1))
            total_loss = masked_lm_loss + next_sentence_loss + svo_loss
            #total_loss = next_sentence_loss
            return total_loss
        else:
            return prediction_scores, seq_relationship_score
...
class BertForSequenceClassificationUOP(BertPreTrainedModel):
    def __init__(self, config, num_labels, max_positions, uop_weight):
...
class BertForSequenceClassificationMTL(BertPreTrainedModel):
    def __init__(self, config, num_labels, max_positions, uop_weight, svo_weight):
...
class BertForSequenceClassificationSVO(BertPreTrainedModel):
    def __init__(self, config, num_labels, svo_weight):
        super(BertForSequenceClassificationSVO, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, svo_ids=None, labels=None):
        sequence_output, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
