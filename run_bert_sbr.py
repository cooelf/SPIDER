...
from pytorch_pretrained_bert.modeling import BertForSequenceClassificationSVO, BertConfig
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, svo_ids, label_id):
...
def convert_examples_to_features(examples, label_list, max_seq_length, max_svo_len,
                                 tokenizer):
...
def main():
    ...
    model = BertForSequenceClassificationSVO.from_pretrained(args.bert_model,
                                                          cache_dir=cache_dir,
                                                          num_labels=num_labels, svo_weight=args.svo_weight)
    if args.fp16:
        model.half()
    model.to(device)
    if args.local_rank != -1:
    ...
    if args.do_train:
        ...
        try:
            ...
            train_features = convert_examples_to_features(
                train_examples, label_list, args.max_seq_length, args.max_svo_len, tokenizer)
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_svo_ids, all_label_ids)
        if args.local_rank == -1:
        ...
        try:
            ...
            eval_features = convert_examples_to_features(
                eval_examples, label_list, args.max_seq_length, args.max_svo_len, tokenizer)
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:
        ...
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_svo_ids, all_label_ids)
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_mask, segment_ids, svo_ids, label_ids = batch

                # define a new function to compute loss values for both output_modes
                loss = model(input_ids, segment_ids, input_mask, svo_ids, label_ids)
            ...
            eval_model = BertForSequenceClassificationSVO.from_pretrained(args.bert_model, state_dict=model_state_dict, num_labels=num_labels, svo_weight=args.svo_weight)
            ...
            for input_ids, input_mask, segment_ids, svo_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                with torch.no_grad():
                    logits = eval_model(input_ids, segment_ids, input_mask, svo_ids, labels=None)
        ...
        model_state_dict = torch.load(output_model_file)
        model = BertForSequenceClassificationSVO.from_pretrained(args.bert_model, state_dict=model_state_dict,
                                                                   num_labels=num_labels, svo_weight=args.svo_weight)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_mask, segment_ids, svo_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                logits = model(input_ids, segment_ids, input_mask, svo_ids, labels=None)
